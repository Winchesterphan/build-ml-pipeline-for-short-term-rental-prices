...
def go(args):
    ...
    dataframe = pd.read_csv(artifact_path, index_col="id")
    min_price = args.min_price
    max_price = args.max_price
    idx = dataframe['price'].between(min_price, max_price)
    dataframe = dataframe[idx].copy()
    logger.info("min price and max price :  %s-%s", min_price, max_price)
    ...
    clean_sample_path = os.path.join(args.tmp_directory, args.output_artifact)
    dataframe.to_csv(clean_sample_path)
    logger.info("Clean sample saved to %s" , clean_sample_path)
    ...
    artifact.add_file(clean_sample_path)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="this step clean data")
